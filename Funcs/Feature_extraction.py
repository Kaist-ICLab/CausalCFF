...
def parallellize_extract_sub(
    ...
    pb = ProgressBar(labels.index.get_level_values('uid').nunique())
    actor = pb.actor

    func = ray.remote(extract_sub).remote 
    results = []

    for pid in labels.index.get_level_values('uid').unique():
...
def extract_sub(
    ...
    #resample or not
    if resample:
        _raw = load(os.path.join(PATH_HOME,f'proc/resampled_data-{_pid}.pkl'))
    else:
        __raw = load(os.path.join(PATH_HOME,f'./proc/clean_data-{_pid}.pkl'))
    ...
    for ema_time in _label.index: 
        ...
        for _s, _e in zip(sub_windows[:-1], sub_windows[1:]):
            ...
            for _d_name in _raw.keys():
                ...
                d_value = _raw[_d_name][_s:_e]
                _d_value = d_value.values.reshape(-1)                  
                if len(_d_value)<1:
...
def parallellize_extract(
    ...
    for pid in tqdm(labels.index.get_level_values('uid').unique()):
        results.append(
            func(
                pid
                , labels
